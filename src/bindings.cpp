...
py::array_t<double> array_view(
  const std::vector<double>& data, py::object base, std::vector<py::ssize_t> shape)
{
...
py::array_t<int> int_array(const std::vector<int>& data)
{
  return py::array_t<int>(
    {static_cast<py::ssize_t>(data.size())}, data.data());
}

std::vector<double> to_vector(const py::array_t<double, py::array::c_style |
...
PYBIND11_MODULE(_core, m)
{
  ...
  py::class_<Node>(m, "Node")
    ...
    .def_property_readonly(
      "width", [](const Node& n) { return std::vector<double>(n.width.begin(), n.width.end()); })
    .def_property_readonly("ijk",
      [](const Node& n) { return std::vector<int>(n.ijk.begin(), n.ijk.end()); });
  ...
  py::class_<XSLibrary>(m, "XSLibrary")
    ...
    .def_property_readonly("delayed",
      ...
      py::return_value_policy::reference_internal)
    .def("set_adf",
      [](XSLibrary& lib, int comp, py::array_t<double, py::array::c_style |
           py::array::forcecast> values, int n_axes) {
        AdfSet& set = lib.adf(comp, n_axes);
        const auto v = to_vector(values);
        if (v.size() != set.value.size()) {
          throw InputError("ADF array must have 2 * n_axes * n_groups entries");
        }
        set.value = v;
      },
  ...
  py::class_<Result>(m, "Result")
    ...
    .def_property_readonly("flux",
      [](py::object self) {
        ...
        const py::ssize_t n = r.power.empty()
          ? 0
          : static_cast<py::ssize_t>(r.power.size());
        const py::ssize_t g = n ? static_cast<py::ssize_t>(r.flux.size()) / n : 0;
  ...
  py::class_<Solver>(m, "Solver")
    ...
    .def("solve_fixed_source",
      [](Solver& s, py::array_t<double, py::array::c_style |
           py::array::forcecast> source, const Settings& settings) {
